...
и инвентарь. Не хватает **содержания**: второго и третьего вида насекомых, мебели,
размножения и правил партии.
...
2. **Смерть сущности обработана наполовину.** `room.tick` убирает сущность из карты по
   ...
   останется на экране.
3. **Мебель невидима.** `Cube` не переопределяет `render` и `blocks`, поэтому
   расставлять её бессмысленно.
4. **Тестами покрыты только предметы и инвентарь** — 12 тестов в `src/test`. Всё,
   что связано с окном, картой и движением, по-прежнему проверяется глазами.
5. **Индексы спрайтов — магические числа** вида `7 + 13 * 32`, разбросанные по коду.
